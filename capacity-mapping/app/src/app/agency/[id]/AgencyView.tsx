 ...
 * The reveal.
 *
 * Folds in a scan session when this tab has one, so walking the scan and then
 * landing here shows the site you just measured rather than the one on file.
 */
export function AgencyView({ agencyId }: { agencyId: string }) {
  const { agency: merged, session } = useAgencyWithScan(agencyId);
    ...
    <main className="flex-1 flex flex-col">
      ...
      <div className="px-5 py-4 border-b border-line">
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          ...
          {session && session.units.length > 0 && (
            <Badge tone="good">Includes today&apos;s scan</Badge>
          )}
        </div>
      </div>
        ...
        {agency.boxes_today === 0 ? (
          <>
            ...
            <p className="text-[13.5px] mt-2.5 leading-relaxed">
              This site has no {ZONE_ADJ[binding.zone_id]} storage at all, so it
              cannot put together a single complete box no matter how much shelf
              space it has.
            </p>
          </>
        ) : (
      ...
      <div className="px-5 py-5 mt-auto">
        ...
        <p className="text-[11.5px] text-muted mt-2.5 text-center leading-relaxed">
          Photograph every place this site keeps food. CareSpace finds the
          storage and sizes it.
        </p>
      </div>
    </main>
  );
